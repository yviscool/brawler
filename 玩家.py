...
class 玩家:
    ...
    def __init__(self, x, y, 翻转, 数据, 精灵序列图, 动画步骤列表):
        ...
        # 更新时间
        self.更新时间 = pygame.time.get_ticks()
        self.奔跑 = False
        self.攻击冷却 = 0
        self.被攻击 = False
        self.存活 = True
    ...
    def 加载图片(self, 精灵序列图, 动画步骤列表):
        ...
        for y, 动画步骤 in enumerate(动画步骤列表):
            ...
            for x in range(动画步骤):
                ...
                临时图片 = 精灵序列图.subsurface(
                    x * self.大小, y * self.大小, self.大小, self.大小)
                临时图片 = pygame.transform.scale(
                    临时图片, (self.大小 * self.缩放, self.大小 * self.缩放))
                临时图片列表.append(临时图片)
    ...
    def 更新(self):
        ...
        if pygame.time.get_ticks() - self.更新时间 > 动画冷却:

            self.更新时间 = pygame.time.get_ticks()
            self.帧索引 += 1

        if self.帧索引 >= len(self.动画列表[self.动作]):

            self.帧索引 = 0

            if self.存活 == False:
                self.帧索引 = len(self.动画列表[self.动作]) - 1
            else:

                if self.动作 == 3 or self.动作 == 4:
                    self.攻击中 = False
                    self.攻击冷却 = 50
